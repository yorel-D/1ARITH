...
def trouver_position(lettre, alphabet):
    ...
    return alphabet.index(lettre_majuscule)


def decalage_chiffrement(i):
    return (i + 6) % 26


def decalage_dechiffrement(i):
    return (i - 6) % 26


def chiffrer_lettre(lettre, alphabet):
    return alphabet[decalage_chiffrement(trouver_position(lettre, alphabet))]


def dechiffrer_lettre(lettre, alphabet):
    return alphabet[decalage_dechiffrement(trouver_position(lettre, alphabet))]


def chiffrer_texte(cylindre, cle, texte):
    texte = enlever_ponctuation(texte)
    if not cle_valide(cle, len(cylindre)):
        return 'Error'
    texte_chiffre = [chiffrer_lettre(texte[i], cylindre[cle[i % len(cle)]]) for i in range(len(texte))]
...
def dechiffrer_texte(cylindre, cle, texte):
    ...
    if not cle_valide(cle, len(cylindre)):
        return 'Error'
    texte_dechiffre = [dechiffrer_lettre(texte[i], cylindre[cle[i % len(cle)]]) for i in range(len(texte))]
    return ''.join(texte_dechiffre)


cylinder = charger_cylindre("cylinderWiki.txt")
key = [7, 9, 5, 10, 1, 6, 3, 8, 2, 4]
encrypted_text = chiffrer_texte(cylinder, key, "RetreatNow")
...
decrypted_text = dechiffrer_texte(cylinder_1ARIT_MP, key_1ARIT_MP, "GRMYSGBOAAMQGDPEYVWLDFDQQQZXXVMSZFS")
